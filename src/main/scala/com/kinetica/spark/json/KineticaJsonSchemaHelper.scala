...
class KineticaJsonSchemaHelper(val loaderConfig: LoaderConfiguration) extends LazyLogging {
  ...
  private val jsonDataFileName = loaderConfig.jsonTemplateFileName
  ...
  def getKineticaTableSchema: KineticaSchemaMetadata = {
    ...
    KineticaJsonSchema(typeInfo, isReplicated)
  }
  ...
  def loadKineticaSchema(path: Path): Option[KineticaSchemaMetadata] = {
    ...
    if (hdfs.exists(targetPath) && hdfs.getFileStatus(targetPath).getLen > 0) {
      ...
      KineticaJsonSchema(contents).kinetica_schema
  ...
  def createTableFromSchema(tableName: String, schemaName: String, kineticaSchemaMetadata: KineticaSchemaMetadata): CreateTableResponse = {
  ...
  def saveKineticaSchema(path: Path): Unit = saveKineticaSchema(path, getKineticaTableSchema)

  def saveKineticaSchema(path: String): Unit = saveKineticaSchema(path, getKineticaTableSchema)
  ...
  //<editor-fold desc="FileSystem Methods">
  ...
  def getValidPath(path: Option[Path]): Path = {
    path match {
      ...
      case _ =>
        throw new RuntimeException("use of Kinetica Json Template requires either a data path or an absolute json filename")
    }
  }
...
object KineticaJsonSchemaHelper {

  def apply(loaderConfig: LoaderConfiguration) = new KineticaJsonSchemaHelper(loaderConfig)

  def apply(parameters: scala.collection.immutable.Map[String, String], sparkSession: SparkSession): KineticaJsonSchemaHelper = {
    val loaderConfig = new LoaderConfiguration(sparkSession.sparkContext, parameters)
    new KineticaJsonSchemaHelper(loaderConfig)
  }
}